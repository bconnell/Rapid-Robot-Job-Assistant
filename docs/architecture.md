...
Rapid Robot Job Assistant is a Manifest V3 Chrome extension with no backend.
...
- Side panel: optional assistant surface for job summary, profile match, field mapping, fill preview, verification status, and notes.
...
AI is disabled by default. The project includes provider interfaces and local manual rules. Any future provider call must create a review preview, redact sensitive text where possible, and wait for explicit user approval.
...
Application sessions are persisted locally for the current page. Sessions store the field preview, notes, fill results, manual verification state, and user-marked status.
